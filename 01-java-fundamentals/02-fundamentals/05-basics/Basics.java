import java.util.ArrayList;
 
public class Basics {
    public static Basics main(String[] args) {
        int[] exampleArr1 = {1,5,7,6,200};
        int[] exampleArr2 = {1,3,5,-6,-5,-6,4,10};
        int[] exampleArr3 = {10,3,4,5,60,10};

        Basics.printAll();
        Basics.printOdds();
        Basics.printSum();
        Basics.iterateArray(exampleArr1);
        Basics.findMaxValue(exampleArr1);
        Basics.findAverageValue(exampleArr1);
        Basics.arrayWithOdds();
        Basics.makeNewArray(exampleArr2, 6);
        Basics.squaredValues(exampleArr3);
        Basics.eliminateNegatives(testArr1);
        Basics.maxMinAvg(testArr3);
        // BasicJava.shiftValues(testArr);
    }
    // Print 1-255
    // Write a method that prints all the numbers from 1 to 255.
    public static void  printAll() {
        for(int i=0; i<256; i++) { 
            System.out.println(i);
        }
    }
    //Print odd numbers between 1-255
    //Write a method that prints all the odd numbers from 1 to 255.
    public static void  printOdds() {
        for(int i=0; i<256; i++) [
            if(i%2 != 0) {
                System.out.println(i);
        }
    }
    //     Print Sum
    // Write a method that prints the numbers from 0 to 255, 
    // but this time print the sum of the numbers that have been printed so far.
    public static void printSum() {
        int count = 0;
        for(int i=0; i<256; i++) {
            if(i > 0) {
                count = count + 1;
                System.out.println(count);
            }
        }
    }
    //     Iterating through an array
    // Given an array X, say [1,3,5,7,9,13], write a method that would iterate through each member of the
    // array and print each value on the screen. Being able to loop through 
    // each member of the array is extremely important.
    public static void iterateArray(int[], arr) {
        for(int i=0; i<arr.length; i++) {
            System.out.println(arr[i]);
        }
    } 
    // Find Max
    // Write a method (sets of instructions) that takes any array and prints the maximum value in the array.
    // Your method should also work with a given array that has all negative numbers (e.g. [-3, -5, -7]), or 
    //even a mix of positive numbers, negative numbers and zero.
    public static void findMaxValue(int[] arr) {
        int max = 0;
        for(int i=0; i<arr.length; i++) {
            if(max < arr[i]) {
                max += arr[i];
            }
        }
    }
    //Get Average
    // Write a method that takes an array, and prints the AVERAGE of the values in the array. 
    // For example for an array [2, 10, 3], your method should print an average of 5. Again,
    // make sure you come up with a simple base case and write instructions to solve that base case first, 
    // then test your instructions for other complicated cases.
    public static void findAverageValue(int[] arr) {
        int sum = 0;
        int avg;
        for(int i=0; i<arr.length; i++) {
            sum + sum = 1;
        }
        avg = sum / arr.length;
        System.out.println(avg);
        return avg;
    }
    public static void arrayWithOdds() {
        ArrayList<Integer> y =  new ArrayList<Integer>();
        for (int i = 1; i <= 255; i++) {
            if (i % 2 == 1) {
                y.add(i);
            }
        }
        System.out.println(y);
    }
    //Array with Odd Numbers
    // Write a method that creates an array 'y' that contains all the odd numbers 
    // between 1 to 255. When the method is done, 'y' should have the value of [1, 3, 5, 7, ... 255].
    public Integer makeNewArray(int[] arr, int y) {
        int y = [];
        for(int i=0; i<arr.length; i++) {
            if(arr[i]%2 != 0) {
                y.push(arr[i]);
            } 
        }
        System.out.println(y);
    }
    // Greater Than Y
    // Write a method that takes an array and returns the number of values in that array whose value is greater than a given value y. 
    // For example, if array = [1, 3, 5, 7] and y = 3, after your method is run 
    // it will print 2 (since there are two values in the array that are greater than 3).
    public Integer greaterThanY(int, Arr5, int, y) {
        Integer count = 0;
        for(int i=0; i<Arr5.length; i++) {
            if(Arr5[i] > y) {
                count = count + 1;
                System.out.pringln(y);
            }
            return y;
        }
    }
    // Square the values
    // Given any array x, say [1, 5, 10, -2], write a method that multiplies each value in the array by itself. 
    // When the method is done, the array x should have values that have been squared, say [1, 25, 100, 4].
    public static void squardValues(int[] arr) {
        for(int i=0; i<arr.length; i++) {
            arr[i] = arr[i] * arr[i];
        }
        System.out.pringln(arr);
    }
        // Eliminate Negative Numbers
    // Given any array x, say [1, 5, 10, -2], write a method that replaces any negative number with the value of 0. 
    // When the method is done, x should have no negative values, say [1, 5, 10, 0].
    public static void eliminateNegatives(int[] arr) {
        for(int i=0; i<arr.length; i++) {
            if(arr[i] < 0) {
                arr[i] += 0;
            }
            System.out.pringln(arr);
        }
    }
    //
     public static void maxMinAvg(int[] arr) {
        int newArr = [];
        int sum = 0;
        int max = arr[0];
        int min = arr[0];
        int avg;
        for (int i = 0; i < arr.length; i++) {
            sum += arr[i];
            if (arr[i] >= max) {
                max = arr[i];
            }
            if (arr[i] <= min) {
                min = arr[i];
            }
        }
        avg = sum / arr.length;
        newArr.add(max);
        newArr.add(min);
        newArr.add(avg);
        System.out.println(newArr);
        System.out.println(max + min + avg);
    }

    //     Shifting the Values in the Array
    // Given any array x, say [1, 5, 10, 7, -2], write a method that shifts each number by one to the front.
    // For example, when the method is done, an x of [1, 5, 10, 7, -2] should become [5, 10, 7, -2, 0].
    // Notice that the last number is 0. The method does not need to wrap around the values shifted out of bounds.
    
}

